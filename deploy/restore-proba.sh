...
# u privremenu bazu restore_test, broj redova po tabeli se uporedi sa živom
# mis_db, pa se restore_test obriše. Pre izmene uporedi sa prod primerkom.
#
...
if [[ "$LIVE_COUNTS" == "$TEST_COUNTS" ]]; then
  ...
  log "RAZLIKA u broju redova (mis_db naspram restore_test):"
...
if [[ "$TEST_TOTAL" -eq 0 ]]; then
  log "GRESKA: restore_test je prazna (0 redova) — dump neupotrebljiv."
  STATUS=1
fi
