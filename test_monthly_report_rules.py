...
# =============================================================================
# Zahtev za otključavanje — lookup po email-u, bez duplih praznih izveštaja
# =============================================================================

class TestRequestApprovalLookup(unittest.TestCase):
    """request_approval mora da nađe izveštaj po email-u (kao load/save), a ne
    samo po imenu — inače pravi duplikat sa employee_email=NULL.  [SPEC-RED]"""

    def _call(self, fetch_queue):
        import timesheet_employee_views as ev
        cursor = RecordingCursor(fetchone_queue=fetch_queue)
        with request_ctx('/request_approval', 'POST', json_body={
                'month': 5, 'year': 2026, 'reason': 'Ispravka sati'}):
            flask.session.update({
                'user': 'zaposleni@nhmbeo.rs',
                'user_email': 'zaposleni@nhmbeo.rs',
                'user_name': 'Test Zaposleni',
                'user_role': 'employee',
            })
    ...
    def test_lookup_uses_email(self):
        cursor = self._call([
            {'id': 44, 'is_verified': True, 'is_locked': True},  # nađen izveštaj
            None,  # nema pending zahteva
            {'id': 1},  # INSERT zahteva RETURNING
        ])
        select_sql = next((q for q, _ in cursor.executed
                           if 'SELECT' in q and 'timesheet_reports' in q), '')
        self.assertIn(
            'employee_email', select_sql,
            'Lookup izveštaja u request_approval mora da koristi employee_email '
            '(konzistentno sa load/save), ne samo employee_name',
        )

    def test_no_bare_report_created_when_report_exists_by_email(self):
        cursor = self._call([
            {'id': 44, 'is_verified': True, 'is_locked': True},
            None,
            {'id': 1},
        ])
        sql = executed_sql(cursor)
        self.assertNotIn(
            'INSERT INTO timesheet_reports', sql,
            'Kada izveštaj postoji, request_approval ne sme da pravi novi red',
        )


# =============================================================================
# PRAVILO 2/3 — šef mora da VIDI pending zahteve svog odseka  [SPEC-RED]
# =============================================================================

class TestPendingListHeadAccess(unittest.TestCase):
    def test_department_head_can_open_pending_requests_page(self):
        import app as museum_app
        client = museum_app.app.test_client()
        with client.session_transaction() as sess:
            sess['user_id'] = 501
            sess['user'] = 'sef.bio@nhmbeo.rs'
            sess['user_email'] = 'sef.bio@nhmbeo.rs'
            sess['user_name'] = 'Šef Biologije'
            sess['user_role'] = 'sef_odeljenja'
            sess['user_department'] = 'Biologija'
            sess['is_department_head'] = True

        cursor = RecordingCursor(fetchall_queue=[[], []], fetchone_queue=[None])
        with patch('timesheet_admin_views.get_postgres_connection',
                   conn_cm_for(cursor), create=True), \
                patch('timesheet_admin_views.render_template',
                      MagicMock(return_value='OK'), create=True):
            resp = client.get('/admin/timesheet/pending',
                              base_url='https://localhost')

        self.assertEqual(
            resp.status_code, 200,
            'Šef odseka odobrava zahteve za otključavanje, pa mora i da ih VIDI '
            '— stranica ne sme biti admin-only (302/403)',
        )
